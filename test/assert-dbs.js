...
const checkViewIndexes = async (db, nbrDocs) => {
  const staleViewUrl = `${url}/${db}/_design/test/_view/view?stale=ok&update_seq=true`;
  const viewResult = await rpn.get({ url: staleViewUrl, json: true });
  viewResult.db = db;

  console.log(staleViewUrl);
  console.log(JSON.stringify(viewResult, null, 2));

  return viewResult.total_rows === nbrDocs;
};
